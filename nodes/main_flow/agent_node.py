...
class AgentNode(Node):
    def __init__(self, max_tool_loops=3, allowed_paths=None,
        ...
        print(f"AgentNode initialized with max_tool_loops={max_tool_loops}, "
    ...
    def exec(self, prep_res, shared):
        ...
        - **downloading cryptocurrency data**: This is a **CRITICAL**
          category. It includes explicit requests to download historical data
          for cryptocurrencies. Keywords indicating this intent are
          "download", "get history", "historical data" combined with
          cryptocurrency symbols (like BTC, ETH, ADA, SOL) and optional
          timeframes (like "daily", "weekly", "1d", "1w", "4h").
          **Examples of crypto download requests:**
          ...
          identify it as a crypto download request.** You should transition
          to a separate 'freqtrade flow' to handle this request.  **Do not
        ...
        - **available data**: If the user asks what data is available or
          what files are present, use the 'directory_listing' tool to check
          the contents of the '{self.data_folder}' folder.
        ...
        - file_read: to read content from a file in the '{self.data_folder}'
          folder
        - file_write: to write content to a file in the '{self.data_folder}'
          folder
        - directory_listing: to list files and directories in the
        ...
        ```yaml
        tool_needed: no  # MUST be 'no' for crypto download requests
        tool_name: <tool_name>  # e.g., directory_listing (if tool_needed
          is yes for non-crypto requests) or None (if tool_needed is no,
          including crypto download requests)
        tool_params:             # Parameters for the tool (if tool_needed
          is yes for non-crypto requests) - MUST be empty for crypto
          download requests
          <param_name_1>: <param_value_1>
        reason: <brief reason for the decision> # MUST clearly state "User
          requested cryptocurrency data download, which is handled by a
          separate flow." for crypto download requests.
        action: <action_indicator> # MUST be 'crypto_download_requested' if
          request is to download crypto data, otherwise None for crypto
          download requests, or other action if tool is needed in this flow.
        ```"""
